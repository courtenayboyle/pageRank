...
#ordering by the highest number of links
cur.execute('''SELECT Count(from_id) AS inbound, old_rank, new_rank, id, url
		FROM Pages JOIN Links ON Pages.id = Links.to_id
    WHERE html IS NOT NULL AND ERROR IS NULL
    GROUP BY id ORDER BY id, inbound''')

#looking for highest & lowest rank
...
for row in cur :
		nodes.append(row)
		rank = row[2]			#pulling out the page rank for each one
...
for row in nodes:
		...
		#print row
		rank = row[2]
		rank = 19 * ( (rank - minrank) / (maxrank - minrank))		#normalizing the rank to be the thickness of the line in our visualization & the size of the ball
